        ...
        {room.members.map((m) => (
          <li key={m.id} className="flex items-center gap-3 rounded-[10px] border border-white/5 bg-white/[0.02] px-3 py-2.5 sm:gap-4">
            <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-sm font-semibold text-white sm:h-10 sm:w-10"
              style={{ backgroundColor: COLOR_MAP[m.color] }}>
              {m.name.charAt(0)}
            </span>
...
export default function RoomDetailPage() {
  ...
  const handleJoin = () => {
    ...
    const message = { type: "JOIN_ROOM", payload: { serverIp: room.serverIp, port: room.port } }
    console.log("[v0] postMessage to Electron:", JSON.stringify(message))
        ...
        <article className="overflow-hidden rounded-[10px] bg-[#0d0d0d] ring-1 ring-white/10">
          ...
          <div className="p-5 sm:p-8">
            <div className="flex items-center gap-2">
              {room.isPrivate ? (
                ...
                <span className="flex items-center gap-1.5 rounded-full bg-white/5 px-2.5 py-1 text-xs font-medium text-white">
                  <span className="h-2 w-2 rounded-full" style={{ backgroundColor: "#bbee00" }} />
                  公开
                </span>
              )}
            </div>
            ...
            <div className="mt-6 grid grid-cols-1 gap-6 md:grid-cols-[1fr_auto]">
              ...
              <div className="flex items-center gap-4 rounded-[10px] border border-white/5 bg-white/[0.02] p-4 md:flex-col md:items-center md:text-center">
                <span className="flex h-14 w-14 shrink-0 items-center justify-center rounded-full text-xl font-bold text-white"
                  style={{ backgroundColor: COLOR_MAP[room.owner.color] }}>
            ...
            <div className="mt-8 flex flex-col gap-3 sm:flex-row">
              {isFull ? (
                <button disabled className="flex-1 cursor-not-allowed rounded-[10px] bg-white/10 px-6 py-3.5 text-base font-semibold text-[#8a8a8a]">
                  房间已满
                </button>
              ) : (
                <button onClick={handleJoin} disabled={joinState !== "idle"}
                  ...
                  style={joinState === "joined"
                    ? { backgroundColor: "#bbee00", color: "#0d0d0d" }
                    : { backgroundImage: "linear-gradient(90deg,#9933ff,#ff33aa)" }}>
              ...
              <button onClick={handleBack}
                className="rounded-[10px] border border-white/30 bg-transparent px-6 py-3.5 text-base font-semibold text-white transition-all duration-200 hover:bg-white/5 active:scale-[0.97]">
                返回列表
              </button>
            </div>
